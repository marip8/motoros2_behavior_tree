...
BT::NodeStatus AddJointsToTrajectoryNode::onTick(const typename sensor_msgs::msg::JointState::SharedPtr& last_msg)
{
  try
  {
    ...
    std::vector<std::string> controller_joint_names = get_parameter<std::vector<std::string>>(node_, CONTROLLER_JOINT_NAMES_PARAM);
                                               ...
                                               const BT::RosNodeParams& params)
{
  ...
  params.nh->declare_parameter<std::vector<std::string>>(motoros2_behavior_tree::AddJointsToTrajectoryNode::CONTROLLER_JOINT_NAMES_PARAM, {});
